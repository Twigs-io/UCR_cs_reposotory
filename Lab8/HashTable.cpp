...
*/
void HashTable::put(const string &s, int score) { //zybooks 26.2
    int index = computeHash(s); //get the index of the string
	for (list<WordEntry>::iterator a = hashTable[index].begin(); a!= hashTable[index].end();) {
        if (a->getWord() == s) { // see if word exists
            a->addNewAppearance(score); //then add score to increase total score and numAppearances
            return; //if exists, return
        }
        else {
            a++; //move on to next if not found
        }
    }
    WordEntry newWordEntry = WordEntry(s, score); //create new Entry or instantiate 
    hashTable[index].push_back(newWordEntry); //push back on appropriate index on the list if it does not exist
}
